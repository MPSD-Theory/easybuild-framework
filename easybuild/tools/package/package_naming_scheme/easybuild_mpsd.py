...
class EasyBuildMPSD(PackageNamingScheme):
    ...
    def name(self, ec):
        ...
        ebsubver = os.environ['MPSD_ARCH']
        if not ebsubver:
            raise ValueError("Environment Variable MPSD_ARCH is undefined.")
        return '%s-%s-%s' % (ec['name'], det_full_ec_version(ec), ebsubver)
